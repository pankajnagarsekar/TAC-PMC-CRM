...
class BackgroundJobEngine:
    ...
    async def _execute_job(self, job_id: str):
        ...
        try:
            ...
            if retry_count < self.MAX_RETRY_ATTEMPTS:
                ...
                logger.info(
                    f"[JOB] Scheduled retry {
                        retry_count +
                        1} for {job_id} in {delay}s")
            else:
    ...
    async def _run_audio_purge_job(
        ...
        logger.info(
            f"[PURGE] Audio: {
                result.deleted_count} recordings deleted")
    ...
    async def _run_drive_retry_job(
        ...
        logger.info(
            f"[RETRY] Drive: {retried} retried, {succeeded} succeeded, {failed} failed")
    ...
    async def _run_compression_retry_job(
        ...
        logger.info(
            f"[RETRY] Compression: {retried} retried, {succeeded} succeeded, {failed} failed")
