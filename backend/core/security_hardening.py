...
class OrganisationAccessError(SecurityError):
    ...
    def __init__(self, user_org: str, resource_org: str):
        super().__init__(
            f"Access denied: User org {user_org} cannot access resource from org {resource_org}")
...
class SecurityHardening:
    ...
    async def verify_resource_access(
        ...
        if resource_org and resource_org != user_organisation_id:
            logger.warning(
                f"[SECURITY] Org access violation: user={user_organisation_id}, resource={resource_org}")
    ...
    def verify_signed_url(
        ...
        # Decode path
        try:
            resource_path = base64.urlsafe_b64decode(
    ...
    def validate_file_path(self, file_path: str) -> bool:
        ...
        if ".." in file_path or file_path.startswith("/"):
            logger.warning(
                f"[SECURITY] Directory traversal attempt: {file_path}")
            return False
