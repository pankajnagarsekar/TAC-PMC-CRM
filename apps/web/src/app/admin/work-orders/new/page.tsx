...
import { ArrowLeft, Save, AlertTriangle, Plus, Trash2, Loader2 } from 'lucide-react';
...
export default function NewWorkOrderPage() {
  ...
  const addLineItem = () => {
    setLineItems([
      ...lineItems,
      { sr_no: lineItems.length + 1, description: '', qty: 0, rate: 0, total: 0 }
    ]);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await api.post(`/api/work-orders/${activeProject._id || activeProject.project_id}`, payload);
      
      if (response.data._warning === 'over_budget') {
        alert('WARNING: This Work Order exceeds the allocated category budget.');
      }
  ...
  if (!activeProject) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        ...
        <p className="text-slate-400">Please select a project from the top navigation to create a Work Order.</p>
        <button onClick={() => router.back()} className="mt-4 text-orange-500 hover:text-orange-400">Go Back</button>
      </div>
    );
  }

  // Filter categories to only Commitment ones
  const commitmentCategories = categories?.filter(c => c.budget_type !== 'fund_transfer') || [];
    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-24 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">Project: {activeProject.project_name}</p>
          </div>
        </div>
        <button
          onClick={handleSave}
          disabled={isSaving || !isGridValid || !formData.category_id || !formData.vendor_id}
          className="bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-orange-500/20"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4 shadow-xl">
          <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest border-b border-slate-800 pb-2">Master Information</h2>
          
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1 uppercase tracking-wider">Category (Commitment)</label>
            <select
            ...
            >
              ...
              {commitmentCategories.map(c => (
                <option key={c._id} value={c._id}>{c.category_name} ({c.code})</option>
              ))}
            </select>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1 uppercase tracking-wider">Vendor</label>
            <select
            ...
            >
              ...
              {vendors?.map(v => (
                <option key={v._id} value={v._id}>{v.name} ({v.gstin || 'No GSTIN'})</option>
              ))}
            </select>
            {fieldErrors.vendor_id && <p className="text-[10px] text-red-500 mt-1 uppercase font-bold">{fieldErrors.vendor_id}</p>}
          </div>
          
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1 uppercase tracking-wider">Description of Work</label>
            <textarea
        ...
        {/* Financial Preview */}
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4 shadow-xl">
          <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest border-b border-slate-800 pb-2">Financial Preview</h2>
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between text-slate-400 pt-2 border-t border-slate-800/50">
              ...
              <span className="font-mono text-white">{formatCurrency(totalBeforeTax)}</span>
            </div>

            <div className="flex justify-between text-slate-400">
              <span>CGST ({formData.cgst}%):</span>
              <span className="font-mono text-white">{formatCurrency(cgstAmount)}</span>
            </div>
            
            <div className="flex justify-between text-slate-400">
              <span>SGST ({formData.sgst}%):</span>
              <span className="font-mono text-white">{formatCurrency(sgstAmount)}</span>
            </div>
            ...
            <div className="flex justify-between items-center text-emerald-500 font-bold pt-2 border-t border-slate-800/50">
              ...
              <span className="font-mono text-lg">{formatCurrency(totalPayable)}</span>
            </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl relative">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
            Bill of Quantities
            {!isGridValid && <AlertTriangle size={14} className="text-red-500" />}
          </h2>
          <button 
            onClick={addLineItem}
