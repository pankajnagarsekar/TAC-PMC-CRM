      ...
      cellRenderer: (p: any) => {
        ...
        return (
          <div className="flex items-center h-full">
            <span
              className={`px-2.5 py-0.5 rounded font-bold text-[10px] uppercase border ${colors[status] || colors["Draft"]}`}
            >
              {status}
            </span>
          </div>
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <button
            ...
            className="admin-only flex items-center gap-2 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-bold transition-colors whitespace-nowrap"
          >
      ...
      {!activeProject ? (
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-12 text-center">
          ...
          <p className="text-slate-400">
            Generate standard commitment orders to reserve budget category
            limits safely.
          </p>
        </div>
      ) : (
