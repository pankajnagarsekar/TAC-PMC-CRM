...
export default function PaymentCertificateDetail({
  ...
  const getCategoryName = (cid: string) =>
    categories?.find((c) => String(c._id || (c as unknown as { id: string }).id) === String(cid))?.category_name || cid;

  const getWoRef = (woid: string) =>
    workOrders.find((w) => String(w._id || (w as unknown as { id: string }).id) === String(woid))?.wo_ref || woid;
