...
import { ColDef, ValueSetterParams, CellValueChangedEvent, ICellRendererParams } from 'ag-grid-community';
...
export default function NewPaymentCertificatePage() {
  ...
  const { data: woResponse } = useSWR(
    activeProject ? `/api/projects/${activeProject.project_id}/work-orders` : null,
    fetcher
  );
  const workOrders: WorkOrder[] = woResponse?.items?.filter((wo: WorkOrder) => wo.status !== 'Cancelled') || [];
  
  const { data: categories } = useSWR<CodeMaster[]>('/api/codes?active_only=true', fetcher);
  const fundCategories = categories?.filter(c => c.budget_type === 'fund_transfer') || [];
  ...
  const { subtotal, retentionAmount, gst, grandTotal, totalPayable } = useMemo(() => {
    const rawSub = lineItems.reduce((sum, item) => sum + (Number(item.total) || 0), 0);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await api.post(`/api/projects/${activeProject.project_id}/payment-certificates`, payload, {
      ...
      router.refresh();
        ...
        if (detail?.errors) {
            ...
            setError(detail || err.message || 'Failed to submit Payment Certificate');
        }
  ...
  const columnDefs: ColDef<PCLineItem>[] = [
    { field: 'sr_no', headerName: 'Sr No', width: 80, editable: false, 
      valueGetter: (params) => (params.node && params.node.rowIndex !== null) ? params.node.rowIndex + 1 : ''
    },
    { field: 'scope_of_work', headerName: 'Scope of Work', flex: 2, editable: true,
      cellEditor: 'agLargeTextCellEditor',
      cellEditorPopup: true
    },
    { field: 'unit', headerName: 'Unit (e.g. sqft)', width: 120, editable: true },
    { 
      field: 'qty', headerName: 'Quantity', width: 120, editable: true, type: 'numericColumn',
    ...
    { 
      field: 'rate', headerName: 'Rate (₹)', width: 150, editable: true, type: 'numericColumn',
      valueSetter: (params: ValueSetterParams<PCLineItem, number>) => {
    ...
    { 
      field: 'total', headerName: 'Total (₹)', width: 150, editable: false, type: 'numericColumn',
  ...
  const handleAddRow = () => {
    setLineItems([
      ...lineItems,
      { id: uuidv4(), sr_no: 0, scope_of_work: '', unit: 'nos', qty: 0, rate: 0, total: 0 }
    ]);
  };

  const handleCellValueChanged = useCallback((event: CellValueChangedEvent<PCLineItem>) => {
    setLineItems(prev => {
      const updated = [...prev];
    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-12 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/admin/payment-certificates" className="p-2 hover:bg-slate-800 rounded-lg transition-colors text-slate-400 hover:text-white">
            <ArrowLeft size={20} />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-white">Create Payment Certificate</h1>
            <p className="text-slate-400 text-sm">Issue new payout request for {activeProject?.project_name}</p>
          </div>
        </div>
        <button
          onClick={handleSave}
          disabled={isSubmitting || !activeProject}
          className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed text-white px-6 py-2 rounded-lg font-medium transition-colors shadow-lg shadow-emerald-500/20"
        >
          {isSubmitting ? <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin"/> : <Save size={18} />}
          Generate Token
        </button>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl">
         ...
         <div className="p-6 border-b border-slate-800/50 bg-slate-900/50">
            <div className="flex gap-4 mb-6 p-1 bg-slate-950 rounded-lg w-fit border border-slate-800">
               <button
                 onClick={() => { setIsWoLinked(true); setSelectedCategoryId(''); }}
               ...
               >
                 <span className="flex items-center gap-2"><Building2 size={16}/> WO-Linked Payment</span>
               </button>
               <button
                 onClick={() => { setIsWoLinked(false); setSelectedWoId(''); }}
               ...
               >
                 <span className="flex items-center gap-2"><FileText size={16}/> Internal Fund Request</span>
               </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
                {isWoLinked ? (
                    <div className="space-y-2">
                        <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Select Work Order Link</label>
                        <select
                            value={selectedWoId}
                        ...
                        >
                            ...
                            {workOrders.map((wo: WorkOrder) => (
                                <option key={wo._id} value={wo._id}>{wo.wo_ref} (Cat: {categories?.find(c => c._id === wo.category_id)?.category_name})</option>
                            ))}
                        </select>
                        {fieldErrors.work_order_id && <p className="text-[10px] text-red-500 mt-1 uppercase font-bold">{fieldErrors.work_order_id}</p>}
                    </div>
                ) : (
                    <div className="space-y-2">
                        <label className="text-xs font-semibold text-amber-400 uppercase tracking-wider">Fund Request Category (Petty/OVH)</label>
                        <select
                        ...
                        >
                            ...
                            {fundCategories.map((c) => (
                                <option key={c._id} value={c._id}>{c.code} - {c.category_name}</option>
                            ))}
                        </select>
                        {fieldErrors.category_id && <p className="text-[10px] text-red-500 mt-1 uppercase font-bold">{fieldErrors.category_id}</p>}
                    </div>
                )}

                <div className="space-y-2">
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Retention Hold %</label>
         ...
         <div className="p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">Scope of Work Calculation</h3>
              <button
