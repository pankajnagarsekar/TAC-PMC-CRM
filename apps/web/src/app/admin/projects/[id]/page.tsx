...
export default function ProjectDetailPage() {
  ...
  const { data: project, mutate: mutateProject } = useSWR<Project>(`/api/projects/${projectId}`, fetcher);
  const { data: financials, mutate: mutateFinancials, isLoading: financialsLoading } = useSWR<DerivedFinancialState[]>(
  ...
  const columnDefs: ColDef<DerivedFinancialState>[] = useMemo(() => [
      ...
      cellRenderer: (params: any) => (
        <div className="flex flex-col justify-center py-1">
          ...
          {params.data.over_commit_flag && (
            <span className="text-[9px] text-red-500 font-bold uppercase tracking-tighter">Budget Overrun</span>
          )}
        </div>
      )
    },
    {
      ...
      onCellValueChanged: async (params: any) => {
        const { _id, approved_budget_amount, version } = params.data;
        if (!_id) return;
        ...
        try {
          ...
          await api.put(`/api/v2/budgets/${_id}`, {
            approved_budget_amount: parseFloat(approved_budget_amount),
            version: version
          });
          mutateFinancials();
        } catch (err: any) {
          ...
          params.node.setDataValue('approved_budget_amount', params.oldValue);
        } finally {
          setSavingId(null);
        }
      }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-4 text-xs text-slate-500">
              <span className="flex items-center gap-1"><MapPin size={12} /> {project.address || 'No address set'}</span>
              <span className="flex items-center gap-1"><Building2 size={12} /> {project.client_name || 'Direct Project'}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            {financials && financials.length === 0 && (
              <button
                onClick={async () => {
                  try {
                    await api.post(`/api/v2/projects/${project._id || project.project_id}/initialize-budgets`, {});
                    mutateFinancials();
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Budget" value={formatCurrency(totalBudget)} icon={TrendingUp} color="#3b82f6" />
        <StatCard title="Total Committed" value={formatCurrency(totalCommitted)} icon={FileText} color="#8b5cf6" />
        <StatCard title="Total Certified" value={formatCurrency(totalCertified)} icon={CheckCircle2} color="#22c55e" />
        <StatCard 
          title="Budget Balance" 
          value={formatCurrency(totalRemaining)} 
          icon={Wallet} 
          color={totalRemaining < 0 ? "#ef4444" : "#F97316"}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-white">Category-wise Financials</h2>
      ...
      <style jsx global>{`
        ...
        .ag-header-cell-label { justify-content: start; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; font-size: 11px; }
      `}</style>
    </div>
  );
}
...
function StatCard({ title, value, icon: Icon, color, alert }: any) {
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-3"
      style={{ borderColor: alert ? 'rgba(239,68,68,0.3)' : '#1e293b' }}>
      <div className="flex items-center justify-between">
