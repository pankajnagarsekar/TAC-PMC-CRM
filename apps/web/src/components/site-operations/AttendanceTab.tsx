...
import { ExternalLink, Calendar, MapPin, Search, X, CheckCircle2 } from "lucide-react";
...
export default function AttendanceTab() {
  ...
  const fetchAttendance = async () => {
    ...
    try {
      ...
      const response = await api.get(`/api/site-operations/projects/${activeProject.project_id}/attendance?${params.toString()}`);
  ...
  const handleVerify = async (logId: string) => {
    try {
      await api.patch(`/api/site-operations/attendance/${logId}/verify`);
  ...
  const formatTime = (dateStr: string) => {
    if (!dateStr) return "--:--";
    return new Intl.DateTimeFormat('en-GB', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }).format(new Date(dateStr));
  };
  ...
  const columnDefs: ColDef[] = useMemo(() => [
    {
      ...
      cellRenderer: (params: any) => (
        <span className="font-semibold text-white">{params.value || "Field Staff"}</span>
      )
    },
    {
      headerName: "Date",
    ...
    },
    {
      ...
      flex: 1,
      cellRenderer: (params: any) => (
        <span className="text-slate-500 font-mono text-xs">{formatTime(params.value)}</span>
      )
    },
    {
      headerName: "Selfie",
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row gap-4 bg-slate-900/50 p-4 rounded-2xl border border-slate-800/50">
        ...
        <p className="text-xs text-slate-500 flex items-center italic">
          <MapPin size={12} className="mr-1" /> All data is GPS verified from mobile app.
        </p>
      </div>
      ...
      {/* Lightbox */}
      {selectedImage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4 animate-in fade-in duration-200" onClick={() => setSelectedImage(null)}>
          ...
          <div className="relative max-w-2xl w-full bg-slate-900 rounded-2xl overflow-hidden border border-slate-800 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 bg-slate-950 border-t border-slate-800 text-center">
              <p className="text-slate-400 text-sm font-medium">Site Personnel Verification Selfie</p>
            </div>
          </div>
        </div>
      )}
