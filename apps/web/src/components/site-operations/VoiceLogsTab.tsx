...
export default function VoiceLogsTab() {
  ...
  const fetchVoiceLogs = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/api/site-operations/projects/${activeProject.project_id}/voice-logs`);
  ...
  const formatFullDate = (dateStr: string) => {
    if (!dateStr) return "N/A";
    return new Intl.DateTimeFormat('en-GB', { dateStyle: 'full', timeStyle: 'short' }).format(new Date(dateStr));
  };
        ...
        {loading ? (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <ChevronRight size={14} className={`mt-1 transition-transform ${selectedLog?._id === log._id ? "text-orange-500 translate-x-1" : "text-slate-700 group-hover:text-slate-500"}`} />
            </div>
            {selectedLog?._id === log._id && (
      ...
      <div className="lg:col-span-8">
        {selectedLog ? (
          <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6 lg:p-8 space-y-8 animate-in fade-in duration-300">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/50 pb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-white tracking-tight">Supervisor Recording</h3>
              ...
              <div className="p-6 rounded-3xl bg-slate-950/80 border border-slate-800 text-sm leading-relaxed text-slate-300 whitespace-pre-wrap min-h-[250px] shadow-inner font-medium">
                {selectedLog.transcribed_text ? (
                  <span className="relative">
                    <span className="text-orange-500/20 text-4xl font-serif absolute -top-4 -left-2">"</span>
                    {selectedLog.transcribed_text}
                    <span className="text-orange-500/20 text-4xl font-serif absolute -bottom-8 -right-2">"</span>
                  </span>
                ) : (
          ...
          <div className="flex flex-col items-center justify-center p-24 text-slate-600 border-2 border-dashed border-slate-800 rounded-[3rem] h-full bg-slate-900/10">
            ...
            <p className="text-sm opacity-60">to view details and transcription</p>
          </div>
        )}
      </div>
