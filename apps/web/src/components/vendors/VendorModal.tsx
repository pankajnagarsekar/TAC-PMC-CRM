...
import { Loader2, ShieldCheck, Mail, Phone, User, Landmark, MapPin } from 'lucide-react';
...
export default function VendorModal({ isOpen, onClose, onSuccess, vendor }: VendorModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (vendor) {
        await api.put(`/api/vendors/${vendor._id || (vendor as any).id}`, formData);
      ...
      <DialogContent className="bg-slate-950 border-slate-900 text-white max-w-lg rounded-2xl p-0 overflow-hidden shadow-2xl">
        <DialogHeader className="p-6 border-b border-slate-900 bg-slate-950/50">
          ...
          <p className="text-slate-500 text-sm mt-1">
            {vendor ? 'Update existing vendor profile and business information.' : 'Enter vendor details to initialize their account.'}
          </p>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="space-y-4">
            <div>
              <label className={labelStyle}>Vendor / Business Name <span className="text-orange-500">*</span></label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, gstin: e.target.value.toUpperCase() })}
                  maxLength={15}
                />
              </div>
              <div>
