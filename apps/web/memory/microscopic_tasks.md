...
- [x] **0.1.1** Add `Vendor` interface to `packages/types/src/index.ts` — fields: `_id`, `organisation_id`, `name`, `gstin`, `contact_person`, `phone`, `email`, `address`, `active_status`, `created_at`, `updated_at`
- [x] **0.1.2** Add `VendorCreate` interface — fields: `name`, `gstin`, `contact_person`, `phone`, `email`, `address`
- [x] **0.1.3** Add `VendorUpdate` interface — all fields optional
- [x] **0.1.4** Update `Project` interface — add `client_id`, `master_original_budget`, `master_remaining_budget`, `threshold_petty`, `threshold_ovh`, `version` fields per DB Schema §2.2
- [x] **0.1.5** Update `ProjectCreate` interface — add `client_id` (required), `threshold_petty`, `threshold_ovh` fields
- [x] **0.1.6** Add `CashTransaction` interface — fields per DB Schema §4.2: `_id`, `project_id`, `category_id`, `amount`, `type` (`'DEBIT'|'CREDIT'`), `bill_reference`, `image_url`, `created_at`
- [x] **0.1.7** Add `VendorLedgerEntry` interface — fields per DB Schema §4.3: `_id`, `vendor_id`, `project_id`, `ref_id`, `entry_type` (`'PC_CERTIFIED'|'PAYMENT_MADE'|'RETENTION_HELD'`), `amount`, `created_at`
- [x] **0.1.8** Add `VoiceLog` interface — fields per DB Schema §5.3: `_id`, `project_id`, `supervisor_id`, `audio_url`, `transcribed_text`, `created_at`
- [x] **0.1.9** Add `AuditLog` interface — fields per DB Schema §6.1: `_id`, `entity_name`, `entity_id`, `previous_state`, `new_state`, `action_type`, `user_id`, `created_at`
- [x] **0.1.10** Add `OperationLog` interface — fields per DB Schema §6.2: `_id`, `operation_key`, `entity_type`, `created_at`
- [x] **0.1.11** Verify all monetary fields in `WorkOrder`, `PaymentCertificate`, `ProjectBudget`, `FundAllocation`, `SiteOverhead` use `number` type (will render as `Decimal128` on backend) — confirm no `string` mismatches
- [x] **0.1.12** Add `ProjectCategoryBudget` interface aligned to DB Schema §3.1 — fields: `_id`, `project_id`, `category_id`, `original_budget`, `committed_amount`, `remaining_budget`, `version`
- [x] **0.1.13** Add `budget_type` field (`'commitment' | 'fund_transfer'`) to `CodeMaster` interface if not present (verify existing)

### 0.2 — Backend Model Hardening (`apps/api/models.py`)

- [x] **0.2.1** Add `Vendor` Pydantic model with Decimal fields for any monetary amounts
- [x] **0.2.2** Add `VendorCreate` Pydantic model
- [x] **0.2.3** Add `VendorUpdate` Pydantic model
- [x] **0.2.4** Add `CashTransaction` Pydantic model with `Decimal` for `amount`
- [x] **0.2.5** Add `VendorLedgerEntry` Pydantic model — all monetary fields as `Decimal`
- [x] **0.2.6** Add `VoiceLog` Pydantic model
- [x] **0.2.7** Add `OperationLog` Pydantic model for idempotency tracking
- [x] **0.2.8** Verify `WorkOrder` model uses `Decimal` for all monetary fields (`subtotal`, `discount`, `total_before_tax`, `cgst`, `sgst`, `grand_total`, `retention_percent`, `retention_amount`, `total_payable`, `actual_payable`, and all `line_items` `qty`, `rate`, `total`)
- [x] **0.2.9** Verify `PaymentCertificate` model uses `Decimal` for all monetary fields
- [x] **0.2.10** Add `version: int = 1` field to `WorkOrder`, `PaymentCertificate`, and `ProjectBudget` models for optimistic concurrency
- [x] **0.2.11** Add `idempotency_key: Optional[str] = None` field to `PaymentCertificate` model
- [x] **0.2.12** Add `status` field with enum validation (`Draft`, `Pending`, `Completed`, `Closed`, `Cancelled`) to `WorkOrder` and `PaymentCertificate` models if not enforced via literal types
- [x] **0.2.13** Add `threshold_petty` and `threshold_ovh` Decimal fields to `Project` model
- [x] **0.2.14** Add `master_original_budget` and `master_remaining_budget` Decimal fields to `Project` model
- [x] **0.2.15** Add `client_id: str` field to `Project` model for client-project linking
...
- [x] **0.3.2** Create `core/idempotency.py` — utility function `check_idempotency(session, operation_key)` that checks `operation_logs` collection and returns `True` if duplicate
- [x] **0.3.3** Create `core/idempotency.py` — utility function `record_operation(session, operation_key, entity_type)` that inserts into `operation_logs`
- [x] **0.3.4** Ensure MongoDB compound indexes exist: `project_id + category_id` on `project_category_budgets`, `work_orders`, `payment_certificates`, `cash_transactions`
- [x] **0.3.5** Ensure MongoDB unique index on `work_orders.wo_ref`
- [x] **0.3.6** Ensure MongoDB unique index on `operation_logs.operation_key`
- [x] **0.3.7** Create `core/indexes.py` — startup function `ensure_indexes(db)` that creates all required indexes

### 0.4 — Financial Service Refactoring (`apps/api/financial_service.py`)

- [x] **0.4.1** Refactor `recalculate_project_code_financials()` — currently aggregates from `petty_cash` collection, but per spec it should aggregate `committed_value` from `work_orders.grand_total` (Commitment Model) and `certified_value` from `payment_certificates.grand_total`
- [x] **0.4.2** Add `recalculate_master_budget(project_id, session)` — sum all `project_category_budgets.original_budget` → update `projects.master_original_budget`; sum all `project_category_budgets.remaining_budget` → update `projects.master_remaining_budget`
- [x] **0.4.3** Add `compute_cash_in_hand(project_id, category_id, session)` — sum all `cash_transactions` (CREDIT - DEBIT) for Fund-Transfer categories (Petty/OVH)
- [x] **0.4.4** Add `check_threshold_breach(project_id, category_id)` — returns boolean if cash_in_hand ≤ threshold
- [x] **0.4.5** Add `compute_15_day_countdown(project_id, category_id)` — calculates days since `fund_allocations.last_pc_closed_date`
- [x] **0.4.6** Fix bug in `recalculate_all_project_financials()` — line 109 references `session=session` parameter but the method signature doesn't accept a `session` keyword argument
...
- [x] **1.1.1** Install `ag-grid-community` and `ag-grid-react` npm packages in `apps/web`
- [x] **1.1.2** Create `apps/web/src/components/ui/FinancialGrid.tsx` — reusable wrapper component around `AgGridReact` with Tailwind theme classes
- [x] **1.1.3** Configure AG Grid default column definitions: right-aligned numeric columns, 2-decimal currency formatter (`₹ X,XX,XXX.XX` Indian numbering), row auto-increment Sr No column
- [x] **1.1.4** Implement keyboard navigation (Tab to next cell, Enter to confirm, Escape to cancel)
- [x] **1.1.5** Implement inline cell validation: `Qty > 0`, `Rate ≥ 0`, `Total = Qty × Rate` row validation
- [x] **1.1.6** Implement row-level validation status indicator (green/red border or icon)
- [x] **1.1.7** Implement "Save blocked if any row invalid" logic — disable Save button and show error count
- [x] **1.1.8** Abstract Enterprise features (copy-paste, range selection) behind a feature flag so Enterprise license can be dropped in later with zero refactor

### 1.2 — Install & Configure Tremor.so

- [x] **1.2.1** Install `@tremor/react` npm package in `apps/web`
- [ ] **1.2.2** Verify Tremor.so Tailwind CSS compatibility with existing tailwind config
- [x] **1.2.3** Create `apps/web/src/components/ui/KPICard.tsx` — reusable financial KPI card (value, label, trend indicator, color-coded status)
- [x] **1.2.4** Create `apps/web/src/components/ui/FinancialChart.tsx` — reusable bar/line chart wrapper for budget vs. expenditure comparisons

### 1.3 — Global Settings Enhancements

- [x] **1.3.1** Audit existing `admin/settings/page.tsx` — verify it has fields for: Company Name, Address, Email, Phone, GST Number, PAN, CGST%, SGST%, WO Prefix, PC Prefix, Invoice Prefix, Terms & Conditions, Currency, Currency Symbol
- [x] **1.3.2** Add **Company Logo Upload** field with image preview (per PRD §5.7)
- [x] **1.3.3** Add **Client Permissions Toggle Board** section — three toggle switches: `can_view_dpr`, `can_view_financials`, `can_view_reports` (per PRD §5.7, Frontend Spec §6)
- [x] **1.3.4** Wire Client Permissions toggles to backend `PATCH /api/settings` endpoint — save to `global_settings.client_permissions`
- [x] **1.3.5** Add backend route `GET /api/settings/client-permissions` to fetch just the client permissions matrix
- [x] **1.3.6** Add backend route `PATCH /api/settings/client-permissions` to update just the client permissions matrix

### 1.4 — Project Context Enforcement Refinements

- [x] **1.4.1** Audit existing `projectStore.ts` — verify project switch triggers full state purge (SWR cache clear, reset all Zustand stores except auth)
- [x] **1.4.2** Implement `window.location.reload()` strategy on project switch (as per conversation history — full page reload for financial data integrity)
- [x] **1.4.3** Verify all API calls in `lib/api.ts` inject `project_id` from project store into requests where required

### 1.5 — Sidebar Navigation Updates

- [x] **1.5.1** Add "Work Orders" menu item in Sidebar (between Projects and Payment Certificates)
- [x] **1.5.2** Add "Vendors" menu item in Sidebar
- [x] **1.5.3** Add "Petty Cash" menu item in Sidebar
- [x] **1.5.4** Add "Site Operations" menu item in Sidebar (group: DPRs, Attendance, Voice Logs)
- [x] **1.5.5** Add "Reports" menu item in Sidebar
- [x] **1.5.6** Ensure Sidebar dynamically hides items for Client role based on `client_permissions`
...
- [x] **3.1.1** Ensure `fund_allocations` collection exists with schema per DB Schema §4.1
- [x] **3.1.2** Auto-create `fund_allocations` entry when project budget is initialized for Petty Cash and OVH categories (`budget_type = 'fund_transfer'`) — `allocation_original = original_budget`, `allocation_remaining = original_budget`, `allocation_received = 0`
- [x] **3.1.3** Create backend route `GET /api/projects/{project_id}/fund-allocations` — returns allocation status per fund-transfer category

### 3.2 — Petty Cash & OVH Expense Module (Backend)

- [x] **3.2.1** Create `apps/api/cash_service.py` — cash transaction business logic
- [x] **3.2.2** Implement "Record Expense" — within transaction:
...
- [x] **3.2.3** Create backend route `POST /api/projects/{project_id}/cash-transactions` — record expense
- [x] **3.2.4** Create backend route `GET /api/projects/{project_id}/cash-transactions` — list all transactions with pagination
- [x] **3.2.5** Create backend route `GET /api/projects/{project_id}/cash-summary` — returns: `cash_in_hand`, `allocation_remaining`, `threshold`, `days_since_last_pc_close`, `threshold_breached` (boolean), `is_negative` (boolean)

### 3.3 — Petty Cash & OVH Module (Frontend — Web CRM)

- [x] **3.3.1** Create route `apps/web/src/app/admin/petty-cash/page.tsx` — Petty Cash & OVH dashboard
- [x] **3.3.2** Dashboard top section: Two KPI cards — "Petty Cash in Hand" and "OVH Cash in Hand" with color-coded values (Red if negative — `#EF4444`, Amber if ≤ threshold — `#F59E0B`, Green otherwise)
- [x] **3.3.3** Dashboard: 15-day countdown timer per category — display days since last PC close. Color: Normal (0–10 days), Amber (11–14 days), Red (15+ days) per Frontend Spec §5.3
- [x] **3.3.4** Implement client-side timer recalculation every 60 seconds (Frontend Spec §5.3)
- [x] **3.3.5** Dashboard: Threshold alert banner — persistent Amber banner when cash ≤ threshold, with "Create PC" quick-link button (Frontend Spec §5.2)
- [x] **3.3.6** Dashboard: Negative cash tooltip/modal explaining the negative state (Frontend Spec §5.1)
- [x] **3.3.7** Create `apps/web/src/components/petty-cash/ExpenseEntryModal.tsx` — form with: Amount, Purpose, Bill/Invoice photo upload, Category selector (Petty/OVH)
- [x] **3.3.8** Wire expense entry to `POST /api/projects/{project_id}/cash-transactions`
- [x] **3.3.9** Show threshold/negative warnings from API response after expense save
- [x] **3.3.10** Transaction history table below — columns: Date, Amount, Purpose, Bill Reference, Type (Debit/Credit), Running Balance

### 3.4 — OCR Integration for Petty Cash

- [x] **3.4.1** Audit existing `admin/ocr/page.tsx` — document what it currently does
  - **Location**: `apps/web/src/app/admin/ocr/page.tsx`
  - **Functionality**: AI-powered invoice/document scanner using GPT-4o Vision
  - **Features**: Drag-drop image/PDF upload, auto-extracts vendor name, invoice number, date, amount, GST, shows confidence score, allows manual editing/verification, creates Payment Certificate directly from extracted data
- [x] **3.4.2** Audit existing backend OCR endpoint in `project_management_routes.py` (OCRBase64Request) — document input/output
  - **Endpoint**: `POST /api/v2/ai/ocr`
  - **Location**: `apps/api/project_management_routes.py:374`
  - **Input**: Multipart file (image/PDF) or base64-encoded JSON body with `image_base64` field
  - **Output**: JSON with `ocr_id`, `raw_text`, `confidence_score`, `extracted_vendor_name`, `extracted_amount`, `extracted_date`, `extracted_invoice_number`, `provider`
- [x] **3.4.3** Wire OCR into Expense Entry Modal: "Scan Invoice" button → captures/uploads image → calls OCR API → auto-fills Amount field from OCR response
- [x] **3.4.4** Show OCR confidence score next to auto-filled amount; allow admin to override
...
- [x] **4.1.1** Create/verify backend route `GET /api/projects/{project_id}/dprs` — list DPRs with status filter (`DRAFT`, `PENDING_APPROVAL`, `APPROVED`)
- [x] **4.1.2** Create/verify backend route `GET /api/dprs/{dpr_id}` — get single DPR with photos and notes
- [x] **4.1.3** Create backend route `PATCH /api/dprs/{dpr_id}/approve` — set status to `APPROVED`, set `approved_by` to current user ID, audit log
- [x] **4.1.4** Create backend route `PATCH /api/dprs/{dpr_id}/reject` — set status to `REJECTED` with rejection reason
- [x] **4.1.5** Ensure DPR list endpoint supports date range filtering

### 4.2 — DPR Review Module (Frontend)

- [x] **4.2.1** Create route `apps/web/src/app/admin/site-operations/page.tsx` — Site Operations hub with tabs: DPRs, Attendance, Voice Logs
- [x] **4.2.2** DPR Tab: List of DPRs with columns: Date, Supervisor, Status, # Photos, Notes preview
- [x] **4.2.3** DPR Tab: Status filter (All / Draft / Pending / Approved)
- [x] **4.2.4** Create route `apps/web/src/app/admin/site-operations/dprs/[id]/page.tsx` — DPR detail view
- [x] **4.2.5** DPR detail: Photo gallery (lightbox viewer) showing all uploaded site photos per Frontend Spec §7
- [x] **4.2.6** DPR detail: Notes section with progress deltas
- [x] **4.2.7** DPR detail: Prominent "Approve" button (green) and "Reject" button (red) — only shown for DRAFT/PENDING status
- [x] **4.2.8** DPR detail: After approval, show "Approved by [Admin Name] on [Date]" badge

### 4.3 — Worker Attendance Verification (Backend)

- [x] **4.3.1** Create/verify backend route `GET /api/projects/{project_id}/attendance` — list attendance records with date filter
- [x] **4.3.2** Create backend route `PATCH /api/attendance/{id}/verify` — set `verified_by_admin = true`

### 4.4 — Worker Attendance Verification (Frontend)

- [x] **4.4.1** Attendance Tab: AG Grid table with columns: Worker Name, Date, Selfie (thumbnail), GPS Map Link (clickable "View on Maps"), Check-in Time, Verified (checkbox)
- [x] **4.4.2** Selfie column: thumbnail image that opens in lightbox on click
- [x] **4.4.3** GPS column: link opens Google Maps at `gps_lat, gps_lng` in new tab
- [x] **4.4.4** "Verify" checkbox triggers `PATCH /api/attendance/{id}/verify`
- [x] **4.4.5** Date range filter on attendance list

### 4.5 — Voice Logs Module (Backend)

- [x] **4.5.1** Create/verify backend route `GET /api/projects/{project_id}/voice-logs` — list voice logs
- [x] **4.5.2** Create/verify backend route `GET /api/voice-logs/{id}` — get single voice log with audio URL and transcription

### 4.6 — Voice Logs Module (Frontend)

- [x] **4.6.1** Voice Logs Tab: list showing Date, Supervisor, Duration/Status, Transcription preview
- [x] **4.6.2** Create `apps/web/src/components/site-operations/AudioPlayer.tsx` — custom HTML5 audio player per Frontend Spec §7
- [x] **4.6.3** Voice Log detail: audio player component + text block displaying backend-transcribed text side by side
...
- [x] **5.1.1** Modify `Sidebar.tsx` — if JWT role is `Client`, fetch `GlobalSettings.client_permissions` and dynamically show/hide menu items per Frontend Spec §6.2
- [x] **5.1.2** Modify `admin/layout.tsx` — if role is `Client`, apply read-only CSS class to entire layout wrapper
- [x] **5.1.3** Create global CSS utility `.client-readonly` — hides all `button[type="submit"]`, disables all `input`, `select`, `textarea`, hides all `.admin-only` elements
- [x] **5.1.4** Add `admin-only` class to all create/edit/delete buttons across all existing modules
- [x] **5.1.5** Test: Client login → only permitted modules visible in sidebar → all inputs disabled → no save/delete buttons visible

### 5.2 — Reporting Engine (Backend)

- [x] **5.2.1** Create `apps/api/reporting_service.py` — reporting aggregation service
- [x] **5.2.2** Implement **Project Summary Report** — MongoDB aggregation pipeline: per-category budget, committed, certified, remaining; master totals; vendor payables summary
- [x] **5.2.3** Implement **WO Tracker Report** — list all WOs with status rollup, total committed per category, linked PC count per WO
- [x] **5.2.4** Implement **PC Tracker Report** — list all PCs with status, linked WO ref, amounts
- [x] **5.2.5** Implement **Petty Cash/OVH Tracker** — cash transactions summary, cash-in-hand per category, allocation remaining
- [x] **5.2.6** Implement **CSA Report** — filter to CSA category only, show budget vs committed vs certified
- [x] **5.2.7** Implement **Date-Range filtered reports** — accept `start_date`, `end_date` query params; filter by `created_at`
- [x] **5.2.8** Create backend route `GET /api/projects/{project_id}/reports/{report_type}` — returns JSON data for each report type
- [x] **5.2.9** All report aggregations computed from MongoDB — zero frontend aggregation per Tech Arch §9

### 5.3 — Reporting Engine (Frontend)

- [x] **5.3.1** Create route `apps/web/src/app/admin/reports/page.tsx` — Reports hub
- [x] **5.3.2** Report selector: dropdown with report types (Project Summary, WO Tracker, PC Tracker, Petty/OVH Tracker, CSA Report)
- [x] **5.3.3** Date range filter: start date, end date pickers
- [x] **5.3.4** "Generate Report" button → calls backend report endpoint → renders results
- [x] **5.3.5** Render **Project Summary** report using Tremor.so: budget vs committed bar chart, category breakdown table, master totals KPI cards
- [x] **5.3.6** Render **WO Tracker** report: table + status distribution donut chart
- [x] **5.3.7** Render **PC Tracker** report: table + amounts bar chart
- [x] **5.3.8** Render **Petty/OVH Tracker**: transactions table + cash-in-hand trend line chart
- [x] **5.3.9** Render **CSA Report**: budget vs actual bar chart + detailed table

### 5.4 — Dashboard Enhancement

- [x] **5.4.1** Refactor existing `admin/dashboard/page.tsx` to use Tremor.so components
- [x] **5.4.2** Add KPI cards: Master Budget, Master Remaining, Total Committed, Total Certified (per PRD §5.6)
- [x] **5.4.3** Add "Vendor Payables" summary card — total outstanding across all vendors
- [x] **5.4.4** Add "Petty Cash Status" card — cash-in-hand value with color coding
- [x] **5.4.5** Add "OVH Status" card — cash-in-hand value with color coding
- [x] **5.4.6** Add "15-Day Countdown" widgets — one for Petty, one for OVH, with day count and color
- [x] **5.4.7** Add "Over-Budget Alerts" section — list categories where `committed_amount > original_budget`
- [x] **5.4.8** Add "Budget vs Committed" bar chart (Tremor.so BarChart)
- [x] **5.4.9** Add "WO Status Distribution" donut chart (Tremor.so DonutChart)
...
> **Implementation Note:** `export_service.py` is located at `apps/api/core/export_service.py` (not `apps/api/`).

- [x] **5.5.1** Install `openpyxl` in `apps/api/requirements.txt`
- [x] **5.5.2** Create `apps/api/core/export_service.py` — export engine (inline HTML templates for PDF)
- [x] **5.5.3** Create Excel template file `apps/api/templates/project_summary.xlsx` based on `majorda templates.xlsx` reference
- [x] **5.5.4** Implement Project Summary Excel export — fill template with data from report aggregation, enforce column positions, formula ranges, protected calculated cells per Tech Arch §10.1
- [x] **5.5.5** Create WO Detail Excel export template and implementation
- [x] **5.5.6** Create PC Detail Excel export template and implementation
- [x] **5.5.7** Create backend route `GET /api/projects/{project_id}/reports/{report_type}/export/excel` — returns downloadable `.xlsx` file
- [x] **5.5.8** Run export via `FastAPI BackgroundTasks` if report is heavy; return `202 Accepted` with job ID; poll for completion

### 5.6 — PDF Export (Backend — WeasyPrint)

> **Implementation Note:** PDF HTML templates are built inline in `apps/api/core/export_service.py` rather than a separate `apps/api/templates/report_pdf.html` file.

- [x] **5.6.1** Install `weasyprint` in `apps/api/requirements.txt`
- [x] **5.6.2** Create `apps/api/core/export_service.py` with inline HTML templates — Jinja2 HTML template styled with CSS to mirror Excel layout per Tech Arch §10.2
- [x] **5.6.3** Include company logo placeholder, letterhead styling, and header section in template
- [x] **5.6.4** Include Terms & Conditions auto-append on final page per Tech Arch §10.2 (use `GlobalSettings.terms_and_conditions`)
- [x] **5.6.5** Implement PDF generation function: populate Jinja2 template → render with WeasyPrint → return bytes
- [x] **5.6.6** Create backend route `GET /api/projects/{project_id}/reports/{report_type}/export/pdf` — returns downloadable `.pdf` file
- [x] **5.6.7** Run via `FastAPI BackgroundTasks` for heavy reports; target < 5s per Tech Arch §14
- [x] **5.6.8** Create standalone WO PDF export: `GET /api/work-orders/{wo_id}/export/pdf`
- [x] **5.6.9** Create standalone PC PDF export: `GET /api/payment-certificates/{pc_id}/export/pdf`

### 5.7 — Export Integration (Frontend)

- [x] **5.7.1** Add "Export to Excel" button on Reports page — triggers Excel download
- [x] **5.7.2** Add "Export to PDF" button on Reports page — triggers PDF download
