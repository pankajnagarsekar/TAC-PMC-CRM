...
async def create_payment_certificate(
    ...
    async with db_manager.transaction_session() as session:
        ...
        # Audit Logging with FULL JSON snapshot
        pc_full = db_manager.from_bson(pc_doc)
        pc_full["_id"] = inserted_id
        await db.audit_logs.insert_one({
            "entity_name": "PaymentCertificate",
            "entity_id": inserted_id,
            "action_type": "CREATE",
            "user_id": user_id,
            "new_state": pc_full,  # FULL JSON snapshot per spec 6.1.2
            "created_at": datetime.now(timezone.utc)
        }, session=session)
...
async def close_payment_certificate(
    ...
    async with db_manager.transaction_session() as session:
        ...
        # Capture full PC state before close for audit
        pc_old_full = db_manager.from_bson(pc)
        ...
        # Get updated PC state for audit
        pc_new = await db.payment_certificates.find_one({"_id": ObjectId(pc_id)}, session=session)
        pc_new_full = db_manager.from_bson(pc_new)

        # Audit Logging with FULL JSON snapshots
        await db.audit_logs.insert_one({
            "entity_name": "PaymentCertificate",
            "entity_id": pc_id,
            "action_type": "CLOSE",
            "user_id": user_id,
            "previous_state": pc_old_full,  # FULL JSON snapshot per spec 6.1.2
            "new_state": pc_new_full,  # FULL JSON snapshot per spec 6.1.2
            "created_at": datetime.now(timezone.utc)
        }, session=session)
