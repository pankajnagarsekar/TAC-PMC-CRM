...
async def check_project():
    ...
    try:
        ...
        if proj:
            ...
            budgets = await db_manager.db.project_category_budgets.find(
                {"project_id": str(proj["_id"])}
            ).to_list(5)
            for b in budgets:
                output.append(
                    f"BID:{b.get('category_id')} O:{b.get('original_budget')}"
                )

            financials = await db_manager.db.financial_state.find(
                {"project_id": str(proj["_id"])}
            ).to_list(10)
            for f in financials:
                output.append(
                    f"FID:{f.get('category_id')} C:{f.get('certified_value')}"
                )

            output.append("WOS:")
            wos = await db_manager.db.work_orders.find(
                {"project_id": str(proj["_id"])}
            ).to_list(10)
            for w in wos:
