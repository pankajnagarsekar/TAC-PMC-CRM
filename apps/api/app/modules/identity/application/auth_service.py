...
class AuthService:
    ...
    def create_access_token(
        ...
        return jwt.encode(
            to_encode, self.config.JWT_SECRET_KEY, algorithm=self.config.ALGORITHM
        )
    ...
    async def decode_token(
        self, token: str, token_type: str = "access", check_revocation: bool = True
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if payload.get("type") != token_type:
                raise AuthenticationError(
                    f"AUTH_ERROR: Invalid purpose ({token_type} expected)."
                )
            ...
            raise AuthenticationError(
                "AUTH_ERROR: Identity cannot be verified or expired."
            )
    ...
    async def login(self, login_data: LoginRequest) -> Token:
        ...
        refresh_payload = jwt.decode(
            refresh_token, self.config.JWT_SECRET_KEY, options={"verify_exp": False}
        )
    ...
    async def refresh_token(self, refresh_token: str) -> Token:
        ...
        token_doc = await self.refresh_repo.find_one(
            {"jti": jti, "user_id": user_id, "is_revoked": False}
        )
        if not token_doc:
        ...
        new_payload = jwt.decode(
            new_refresh, self.config.JWT_SECRET_KEY, options={"verify_exp": False}
        )
        await self.refresh_repo.create(
            {
                "jti": new_payload["jti"],
