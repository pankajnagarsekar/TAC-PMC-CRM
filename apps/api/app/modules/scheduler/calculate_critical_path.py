...
def calculate_cpm():
    try:
        ...
        if not tasks_input:
            print(json.dumps({"tasks": [], "critical_path": [], "total_duration_days": 0}))
            return
        ...
        for t in tasks_input:
            ...
            task_map[tid] = {
                ...
                "predecessors": [str(p.get("task_id") if isinstance(p, dict) else p) for p in t.get("predecessors", [])],
                "successors": [],
                "es": None,
                "ef": None,
                "ls": None,
            ...
            task_map[tid]["preds"] = list(set(task_map[tid]["dependencies"] + task_map[tid]["predecessors"]))
        ...
        if len(topo_order) < len(task_map):
            print(json.dumps({"error": "Circular dependency detected in schedule graph."}))
            return
        ...
        if topo_order:
            final_ef = max((task_map[tid]["ef"] for tid in task_map), default=project_start)
