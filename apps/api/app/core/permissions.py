...
class PermissionChecker:
    ...
    async def validate_active_user(user: dict):
        ...
        if not user:
            raise HTTPException(
                status_code=401, detail="INVALID_SESSION: User record not found."
            )
    ...
    async def check_write_access_with_role(self, user: dict, project_id: str = None):
        ...
        if role == "Supervisor":
            raise HTTPException(
                status_code=403,
                detail="ENVIRONMENT_ERROR: Supervisors restricted to Mobile.",
            )

        if role == "Client":
            raise HTTPException(
                status_code=403,
                detail="DOMAIN_ERROR: Client role is strictly read-only.",
            )
    ...
    async def check_web_crm_access(user: dict):
        ...
        if user.get("role") == "Supervisor":
            raise HTTPException(
                status_code=403,
                detail="ACCESS_DENIED: Supervisor must use Mobile interface.",
            )
        return True
    ...
    async def check_client_readonly(user: dict):
        ...
        if user.get("role") == "Client":
            raise HTTPException(
                status_code=403, detail="ACCESS_DENIED: Client role is read-only."
            )
        return True
