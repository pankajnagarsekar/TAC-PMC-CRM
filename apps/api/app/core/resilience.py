    ...
    async def call(self, func: Callable, *args, **kwargs) -> Any:
        if self.state == "OPEN":
            if time.time() - self.last_failure_time > self.recovery_timeout:
                ...
                logger.info(
                    f"CIRCUIT_BREAKER [{self.name}]: Transitioning to HALF_OPEN"
                )
            else:
                raise HTTPException(
...
class NonceGuard:
    ...
    async def verify(self, nonce: str, user_id: str):
        ...
        if not nonce:
            raise HTTPException(
                status_code=400,
                detail="NONCE_REQUIRED: X-Request-Nonce header is missing.",
            )

        existing = await self.db.request_nonces.find_one(
        ...
        await self.db.request_nonces.insert_one(
            {"nonce": nonce, "user_id": user_id, "used_at": time.time()}
        )
