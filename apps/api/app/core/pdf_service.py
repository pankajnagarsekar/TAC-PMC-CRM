...
class DPRPDFGenerator:
    ...
    def generate_pdf(
        ...
        for idx, image in enumerate(images):
            ...
            story.extend(
                self._build_image_page(image, idx + 1, len(images), project_data)
            )
        doc.build(story)
    ...
    def _build_page_one(
        ...
        elements.append(
            Paragraph(f"{project_name} ({project_code})", self.styles["DPRSubtitle"])
        )
    ...
    def _build_image_page(
        ...
        elements = []
        elements.append(
            Paragraph(f"Photo {image_num} of {total_images}", self.styles["Caption"])
        )
        ...
        if image_b64:
            try:
                ...
                pass
        elements.append(
            Paragraph(image_data.get("caption", ""), self.styles["Caption"])
        )
        return elements
