...
class ConsistencyGuardian:
    ...
    async def find_zombies(self):
        ...
        zombie_pcs = await self.db.payment_certificates.aggregate(pipeline).to_list(
            length=100
        )
        ...
        if zombie_pcs:
            ...
            for pc in zombie_pcs:
                await self.alert_service.raise_alert(
                    ...
                    severity="high",
                    message=(
                        f"Payment Certificate {pc.get('pc_ref')} exists without "
                        f"a valid Project ID {pc.get('project_id')}"
                    ),
    ...
    async def verify_financial_sync(self, project_id: str, organisation_id: str):
        ...
        master = await self.db.financial_state.find_one(
        ...
        if category_sum:
            ...
            if actual != expected:
                logger.critical(
                    f"INTEGRITY_FAILURE: Project {project_id} Master ({expected}) != Category Sum ({actual})"
                )
                await self.alert_service.raise_alert(
                    ...
                    severity="critical",
                    message=(
                        f"Financial divergence in Project {project_id}. "
                        f"Master committed {expected} vs Sum of categories {actual}."
                    ),
