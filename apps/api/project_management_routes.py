...
async def run_ocr(
    ...
    """
    Run OCR on uploaded document (Multipart) or base64 (JSON).

    Extracts vendor, invoice number, date, amount.
    Returns flattened structure for mobile compatibility.
    """
    user = await permission_checker.get_authenticated_user(current_user)
    ...
    if "application/json" in content_type:
        try:
            ...
            ocr_req = OCRBase64Request(**body)
            content = base64.b64decode(ocr_req.image_base64)
            project_id = ocr_req.project_id or project_id
            file_type = "image/jpeg" # Assume jpeg for base64 from mobile
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Invalid JSON/Base64: {str(e)}")
    elif file:
    ...
    try:
        ...
        # Map to mobile-friendly OCRResult interface
        structured = result.get("structured", {})
        return {
            "ocr_id": result.get("ocr_id"),
            "raw_text": result.get("raw_text", ""),
            "confidence_score": result.get("confidence", 0),
            "extracted_vendor_name": structured.get("vendor_name"),
            "extracted_amount": structured.get("amount"),
            "extracted_date": structured.get("invoice_date"),
