...
async def list_projects(
    ...
    """List all projects for the organisation with pagination"""
    user = await permission_checker.get_authenticated_user(current_user)
    # Return projects for the user's organisation
    projects = await db.projects.find({"organisation_id": user["organisation_id"]}).skip(skip).limit(limit).to_list(length=limit)
...
async def create_project(project_data: ProjectCreate, current_user: dict = Depends(get_current_user)):
    ...
    project_dict["status"] = project_data.status or "active"
    
    result = await db.projects.insert_one(project_dict)
    project_id = str(result.inserted_id)
    project_dict["_id"] = result.inserted_id
    
    # Ensure project_id string field is also set for legacy compatibility
    await db.projects.update_one({"_id": result.inserted_id}, {"$set": {"project_id": project_id}})
    ...
    await audit_service.log_action(
        ...
        action_type="CREATE",
        user_id=user["user_id"],
        new_value={"project_name": project_data.project_name, "project_code": project_data.project_code}
    )
...
async def get_project(project_id: str, current_user: dict = Depends(get_current_user)):
    ...
    user = await permission_checker.get_authenticated_user(current_user)
    # Try finding by _id or project_id string field
    project = await db.projects.find_one({
        "$or": [{"_id": ObjectId(project_id)}, {"project_id": project_id}],
        "organisation_id": user["organisation_id"]
    })
...
async def update_project(project_id: str, project_data: ProjectUpdate, current_user: dict = Depends(get_current_user)):
    ...
    result = await db.projects.find_one_and_update(
        {"$or": [{"_id": ObjectId(project_id)}, {"project_id": project_id}], "organisation_id": user["organisation_id"]},
        {"$set": update_data},
        return_document=True
    )
...
# ============================================
# CODE MASTER (CATEGORY) ENDPOINTS
# ============================================

@api_router.get("/codes", response_model=List[CodeMaster])
async def list_codes(current_user: dict = Depends(get_current_user)):
    """List all budget categories/codes"""
    user = await permission_checker.get_authenticated_user(current_user)
    codes = await db.code_masters.find({}).to_list(length=200)
    return [serialize_doc(c) for c in codes]

@api_router.post("/codes", response_model=CodeMaster, status_code=status.HTTP_201_CREATED)
async def create_code(code_data: CodeMasterCreate, current_user: dict = Depends(get_current_user)):
    """Create a new budget category/code"""
    user = await permission_checker.get_authenticated_user(current_user)
    await permission_checker.check_admin_role(user)
    ...
    result = await db.code_masters.insert_one(code_dict)
    code_dict["_id"] = result.inserted_id
    
    return serialize_doc(code_dict)

@api_router.put("/codes/{code_id}", response_model=CodeMaster)
async def update_code(code_id: str, code_data: CodeMasterUpdate, current_user: dict = Depends(get_current_user)):
    """Update a budget category/code"""
    user = await permission_checker.get_authenticated_user(current_user)
    await permission_checker.check_admin_role(user)
    ...
    result = await db.code_masters.find_one_and_update(
...
async def delete_code(code_id: str, current_user: dict = Depends(get_current_user)):
    """Soft delete a code by setting active_status to false, if not referenced"""
    user = await permission_checker.get_authenticated_user(current_user)
    await permission_checker.check_admin_role(user)
    
    # Check if code is referenced in project_category_budgets
    budget_count = await db.project_budgets.count_documents({"code_id": code_id})
    # Check work orders
    wo_count = await db.work_orders.count_documents({"line_items.category_id": code_id})
    # Check payment certificates
    pc_count = await db.payment_certificates.count_documents({"line_items.category_id": code_id})
    
    if budget_count > 0 or wo_count > 0 or pc_count > 0:
        raise HTTPException(status_code=400, detail="Cannot delete category that is referenced by budgets, work orders, or payment certificates")
...
@api_router.get("/projects/{project_id}/budgets", response_model=List[ProjectBudget])
async def list_project_budgets(project_id: str, current_user: dict = Depends(get_current_user)):
    """List all budgets for a project"""
    user = await permission_checker.get_authenticated_user(current_user)
    budgets = await db.project_budgets.find({"project_id": project_id}).to_list(length=100)
    return [serialize_doc(b) for b in budgets]

@api_router.post("/projects/{project_id}/budgets", response_model=ProjectBudget, status_code=status.HTTP_201_CREATED)
async def create_project_budget(project_id: str, budget_data: ProjectBudgetCreate, current_user: dict = Depends(get_current_user)):
    ...
    await financial_service.recalculate_project_code_financials(project_id, budget_data.code_id)
    
    return serialize_doc(budget_dict)

@api_router.put("/budgets/{budget_id}", response_model=ProjectBudget)
async def update_project_budget(budget_id: str, budget_data: ProjectBudgetUpdate, current_user: dict = Depends(get_current_user)):
    ...
    result = await db.project_budgets.find_one_and_update(
    ...
    # Recalculate financials
    await financial_service.recalculate_project_code_financials(result["project_id"], result["code_id"])
