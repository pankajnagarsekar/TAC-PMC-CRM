...
class SnapshotImmutableError(Exception):
    ...
    def __init__(self, snapshot_id: str, action: str):
        super().__init__(
            f"Snapshot {snapshot_id} is immutable. {action} is blocked.")
...
class SnapshotEngine:
    ...
    async def create_snapshot(
        ...
        if report_type not in self.REPORT_TYPES:
            raise ValueError(
                f"Invalid report_type: {report_type}. Must be one of {
                    self.REPORT_TYPES}")
        ...
        logger.info(
            f"[SNAPSHOT] Created: {snapshot_id} type={report_type} checksum={checksum[:16]}...")
    ...
    async def get_snapshot(
        ...
        if verify_checksum:
            ...
            if computed_checksum != snapshot["checksum_hash"]:
                logger.error(
                    f"[SNAPSHOT] Checksum mismatch for {snapshot_id}!")
                raise ValueError(
                    f"Snapshot {snapshot_id} checksum verification failed - data may be corrupted")
        ...
        pc_pipeline = [
            {
                "$match": {
                    "project_id": project_id, "status": {
                        ...
                        "$group": {
                            "_id": None, "total_count": {
                                "$sum": 1}, "total_certified": {
                                    "$sum": "$current_bill_amount"}}}]
