...
class SnapshotService:
    ...
    async def get_global_settings(
            self, organisation_id: str) -> Dict[str, Any]:
        """
    ...
    async def verify_checksum(
        ...
        current_data_checksum = compute_json_checksum(
...
class DocumentLockService:
    ...
    async def unlock_document(
        ...
        if result.modified_count > 0:
            logger.warning(
                f"[LOCK] Document unlocked: {collection}/{document_id} reason={reason}")
            return True
...
async def build_dpr_snapshot(
    ...
    try:
        project_query = {"_id": ObjectId(project_id)} if len(
            str(project_id)) == 24 else {"project_id": project_id}
