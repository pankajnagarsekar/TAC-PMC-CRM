...
async def create_vendor(
    ...
    await audit_service.log_action(
        ...
        new_value={"name": vendor_data.name}
    )
...
async def get_vendor_ledger(
    ...
    vendor = await db.vendors.find_one({"_id": ObjectId(vendor_id), "organisation_id": user["organisation_id"]})
    if not vendor:
        raise HTTPException(status_code=404, detail="Vendor not found")

    entries = await db.vendor_ledger.find({"vendor_id": vendor_id}).sort("created_at", -1).to_list(length=500)
